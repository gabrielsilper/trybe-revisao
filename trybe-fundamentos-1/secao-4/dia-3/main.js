...
let elementWithClassTech = firstLi;

function addClassTech(event) {
  elementWithClassTech.classList.remove('tech');
  event.target.className = 'tech';
  elementWithClassTech = event.target;
}
...
input.addEventListener('keyup', () => {
  elementWithClassTech.innerText = input.value;
});
...
myWebpage.addEventListener('dblclick', () => {
  window.open('https://www.youtube.com/watch?v=gm_C-0Y69dQ');
});
...
myWebpage.addEventListener('mouseover', (event) => {
  event.target.style.color = `rgb(${getRandomNumber()}, ${getRandomNumber()}, ${getRandomNumber()})`;
});
